...
- Fill missing — copies originals to `processed/` (skipped) or `processed/errors/` (errors) so every input has a matching output entry
...
- **Fill missing** checkbox (above Resize max width, enabled by default) — when
  enabled, skipped files are copied with original extension into `processed/`,
  and failed/errored files into `processed/errors/`, so every input file has a
  matching output entry.
- **Fixed window size** — 620x624, not resizable. Resizing would break the
