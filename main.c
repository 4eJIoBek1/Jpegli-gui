...
typedef struct { char *path; int is_anim_gif; int is_jpeg; int s_h, s_v; int prog; int cb_h, cb_v; } FileEntry;
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
    switch (msg) {
        case WM_CREATE: {
            ...
            HWND grp = CreateWindowExA(0, "BUTTON", "Manual Configuration", WS_CHILD|WS_VISIBLE|BS_GROUPBOX, 10, y, rc.right-20, 158, hWnd, 0, g_hInst, NULL);
        ...
        case WM_SIZE: {
            RECT rc; GetClientRect(hWnd, &rc);
            if (g_hDropStatic) SetWindowPos(g_hDropStatic, 0, 10, 176, rc.right-20, 100, SWP_NOZORDER);
            if (g_hProgress) SetWindowPos(g_hProgress, 0, 10, 284, rc.right-240, 22, SWP_NOZORDER);
            if (g_hProcessBtn) SetWindowPos(g_hProcessBtn, 0, rc.right-220, 282, 100, 26, SWP_NOZORDER);
            if (g_hClearBtn) SetWindowPos(g_hClearBtn, 0, rc.right-110, 282, 100, 26, SWP_NOZORDER);
            if (g_hLog) SetWindowPos(g_hLog, 0, 10, 314, rc.right-20, rc.bottom-349, SWP_NOZORDER);
        ...
        case WM_COMMAND: {
            ...
            if (id == IDC_CLEAR_BTN && code == BN_CLICKED) {
                ...
                for (int i = 0; i < g_fileCount; i++) free(g_files[i].path);
                g_fileCount = 0;
                set_status("Ready");
                ShowWindow(g_hClearBtn, SW_HIDE);
